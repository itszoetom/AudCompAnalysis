...
svm_types = ["linear", "rbf"]  # Both linear and nonlinear SVM
colors = {
...
hyperparameter_results = {'linear': {}, 'rbf': {}}
all_results = {'linear': [], 'rbf': []}
boxplot_data = {'linear': {}, 'rbf': {}}

# HYPERPARAMETER TUNING PHASE
for svm_type in svm_types:
    print(f"\n{'=' * 60}")
    print(f"HYPERPARAMETER TUNING: {svm_type.upper()} SVM")
    print(f"{'=' * 60}")
    ...
    for stim in stim_types:
        ...
        for brainRegion in uniqRegions:
            print(f"   -> {response_window} - {brainRegion} - {svm_type}")
            ...
            hyperparameter_results[svm_type][key] = {
...
for svm_type in svm_types:
    for stim in stim_types:
        ...
        for row_idx, brainRegion in enumerate(uniqRegions, start=1):
            ...
            if key in hyperparameter_results[svm_type]:
                c_values = hyperparameter_results[svm_type][key]['c_values']
                accuracies = hyperparameter_results[svm_type][key]['accuracies']
                ...
                print(f"{svm_type} - {key}: Best C = {best_c:.3f}, Accuracy = {best_acc:.3f}")

        # Update layout
        fig.update_xaxes(type='log', title_text='C')
        fig.update_yaxes(title_text='Accuracy')
        fig.update_layout(
            title=f"Hyperparameter Tuning for {stim} ({svm_type.upper()} SVM)",
            height=400 * n_regions,
            width=1000
        )

        # Save figure
        fig.write_html(f"{save_dir}hyperparameter_tuning_{stim}_{svm_type}.html")
        print(f"Saved hyperparameter tuning plot for {stim} ({svm_type})")

# Save hyperparameter results to CSV
hp_results_list = []
for svm_type in svm_types:
    for key, data in hyperparameter_results[svm_type].items():
        parts = key.split('_')
        # Handle multi-word region names
        stim = parts[0]
...
for svm_type in svm_types:
    print(f"\n{'=' * 60}")
    print(f"FINAL ANALYSIS: {svm_type.upper()} SVM")
    print(f"{'=' * 60}")
    ...
    for stim in stim_types:
        ...
        for col_idx, brainRegion in enumerate(uniqRegions, start=1):
            ...
            with tqdm(total=total_pairs,
                      desc=f"{stim} | {response_window} | {brainRegion} | {svm_type} (C={best_c:.3f})",
                      leave=True) as pbar:
                for i1, stim1 in enumerate(uniqStims):
                    mask1 = stimArray == stim1
                    resp1 = brain_resp_array[:, mask1]  # neurons, trials
            ...
            upper_tri = np.triu_indices(len(uniqStims), k=1)
            boxplot_data[svm_type][f"{brainRegion}_{stim}_{response_window}"] = svm_stim_vals[upper_tri].flatten()

            # Create visualization for best and worst classifier pairs (only for linear SVM)
            if svm_type == 'linear' and len(pair_accuracies) >= 2 and brain_resp_array.shape[0] >= 2:
                ...
                fig_viz.update_layout(height=500, width=1000,
                                      title=f"Classifier Examples: {stim} - {brainRegion} - {response_window} (LINEAR SVM, C={best_c:.3f})")
                fig_viz.write_html(f"{save_dir}classifier_examples_{stim}_{brainRegion}_{response_window}_linear.html")
        ...
        fig_sub.update_layout(title=f"SVM Accuracy Heatmaps for {stim} ({svm_type.upper()}, Optimized C)",
                              height=600, width=400 * len(uniqRegions))
        fig_sub.write_html(f"{save_dir}SVM_heatmaps_{stim}_{svm_type}_optimized.html")
        print(f"Saved {stim} heatmaps with optimized C ({svm_type})")

# Save all results for later replotting (separate files for each SVM type)
for svm_type in svm_types:
    results_df = pd.DataFrame(all_results[svm_type])
    results_save_path = os.path.join(save_dir, f"svm_pairwise_results_{svm_type}.csv")
    results_df.to_csv(results_save_path, index=False)
    print(f"\nSaved all pairwise SVM results for {svm_type} to {results_save_path}")