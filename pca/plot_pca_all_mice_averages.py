"""Population PCA plots after averaging repeated trials within each stimulus."""
...
def main() -> None:
    ...
    for sound_type in list_available_sound_types():
        ...
        fig.suptitle(
            f"{sound_type} averaged PCA (matched to {min_neurons} neurons per region)",
            fontsize=16,
        )
        scree_ymax = get_sound_scree_ymax(sound_type, average_trials=True)
        ...
        for row_index, brain_area in enumerate(brain_regions):
            for window_index, window_name in enumerate(WINDOW_ORDER):
                ...
                plot_pca_scatter(scatter_ax, averaged_dataset, summary, title)
                plotted_panels += 2
        ...
        fig.savefig(get_figure_dir() / f"{sound_type}_population_pca_averaged.png", dpi=200)
        plt.close(fig)
