...
Runtime: roughly 30 to 40 minutes for the full pipeline.

## Scripts

- `plot_linear_svm.py` (Figures 9–11, S9–S13): all linear SVM figures, including heatmaps, regional boxplots, natural-sound within/between boxplots, C-tuning curves, and decision boundary examples.
...
- `discriminability_analysis.py`: shared helpers (`run_method_analysis`, `draw_heatmap_grid`, `plot_region_boxplots`, and others).
...
4. `LinearSVC` with regularization C tuned over 20 log-spaced values from 10⁻⁵ to 10⁴, fit per sound, region, and window.
