"""Create per-session ridge R2 distributions for each sound type across regions and spike windows."""
...
def main() -> None:
    ...
    for sound_type in ("speech", "AM", "PT", "naturalSound"):
        ...
        fig.suptitle(f"{sound_type} per-session ridge $R^2$ (30 neurons, 100 subsamples)", fontsize=16, fontweight="bold")
        for row_index, brain_area in enumerate(brain_regions):
            for col_index, window_name in enumerate(WINDOW_ORDER):
                ...
                sns.stripplot(data=panel_df, x="Target", y="R2 Test", color="black", size=3, alpha=0.4, ax=ax)
